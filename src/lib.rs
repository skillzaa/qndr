...
fn get_alphanumeric(sample:&str )->String{
...
fn get_non_alphabets(sample:&str )->String{
...
fn get_non_numbers(sample:&str )->String{
    let mut numbers = String::from(""); 
    for c in sample.chars() { 
        let r = !c.is_numeric();
        match r {
...
/// The get_non_alphanumeric fn takes &str and returns (as a new
/// string) of all the non *Alpha-Numeric* chars.
fn get_non_alphanumeric(sample:&str )->String{
...
fn numbers_with_symbols(sample:&str,allowed_symbols:&str)->bool{
...
/// The alphabets_with_symbols fn will allow just alphabets and few
/// selected symbols e.g ! , @ etc. 
/// It takes a &str (the sample string) and another &str with allowed
/// symbols. It will return true only if the sample string has only
/// alphabets and the allowed symbols provided.

fn alphabets_with_symbols( sample:&str,allowed_symbols:&str )->bool{
    let non_alpha = get_non_alphabets(&sample);
    let result = check_string_for_allowed_chars
    (&non_alpha,&allowed_symbols);
    result
}
fn alphanumeric_with_symbols( sample:&str ,allowed_symbols:&str  )->Result<bool,Error>{
...
fn no_caps( sample:&str )->Result<bool,Error>{
...
fn only_caps( sample:&str )->Result<bool,Error>{
    for i in sample.chars(){
        if !i.is_ascii_uppercase() {return None}
    }
    Some(true)
...
fn begin_with( sample:&str,begin_char:char )->bool{
...
fn end_with( sample:&str,end_char:char )->bool{
...
fn begin_with_number( sample:&str )->bool{
    let first = sample.chars().nth(0);
    match first {
        Some(l)=>{
            if l.is_numeric() { 
...
fn end_with_number( sample:&str )->bool{
    let last = sample.chars().rev().nth(0);
    match last {
        Some(l)=>{
            if l.is_numeric() { 
...
fn begin_with_alphabet( sample:&str )->bool{
    let first = sample.chars().nth(0);
    match first {
        Some(l)=>{
            if l.is_alphabetic() { 
...
fn end_with_alphabet( sample:&str )->bool{
    let last = sample.chars().rev().nth(0);
    match last {
        Some(l)=>{
            if l.is_alphabetic() { 
...
fn begin_with_alphanumeric( sample:&str )->bool{
    let first = sample.chars().nth(0);
    match first {
        Some(l)=>{
...
fn end_with_alphanumeric( sample:&str )->bool{
    let last= sample.chars().rev().nth(0);
    match last {
        Some(l)=>{
            if l.is_alphanumeric() { 
...
fn string_to_vec (incomming:&String)->Vec<char>{
...
fn check_string_for_allowed_chars(data:&str,allowed_chars:&str)->bool{
    for i in data.chars(){
...
fn remove_chars(sample:&str,subtract:&str)->String{
